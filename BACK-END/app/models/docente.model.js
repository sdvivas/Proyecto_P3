...
Tarea_Asignatura.buscarMaterias = (cod_docente, result) => {
    sql.query("SELECT TAREA.COD_NIVEL_EDUCATIVO, NIVEL.NOMBRE FROM TAREA_ASIGNATURA TAREA INNER " +
        "JOIN NIVEL_EDUCATIVO NIVEL ON TAREA.COD_NIVEL_EDUCATIVO = NIVEL.COD_NIVEL_EDUCATIVO WHERE TAREA.COD_DOCENTE = 3 " +
...
Tarea_Asignatura.listarMaterias = (cod_docente, cod_paralelo, result) => {
    sql.query(`SELECT a.NOMBRE, a.COD_ASIGNATURA FROM ASIGNATURA a
        ...
        AND sp.COD_DOCENTE = ${cod_docente} AND sp.COD_PARALELO = '${cod_paralelo}' AND sp.COD_QUIMESTRE=1 
        GROUP BY sp.COD_ASIGNATURA;`, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result("hola" + err, null);
            return;
